...
`)
    ...
    expect(result.code).toContain(
      `const nextTarget = nextValue == null ? null : ((input) => ({`,
    )
...
`)
    ...
    expect(result.code).toContain(
      `const __forwardedRef = customRef<HTMLInputElement | null>`,
    )
    expect(result.code).toContain(
      `const props = defineProps<{ __forwarded_ref__?: Ref<InputHandle | null> | ((value: any) => void) }>()`,
    )
...
`,
    ...
    expect(result.code).toContain(
      `<A :__forwarded_ref__="(value) => a = value" />`,
    )
    expect(result.code).toContain(
...
`,
    ...
    expect(result.code).toContain(
      `:__forwarded_ref__="(value) => input = value"`,
    )
