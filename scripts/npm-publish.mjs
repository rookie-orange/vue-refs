...
const npmArgs = ["--cache", "./.npm-cache", "--registry", "https://registry.npmjs.org/", "publish"];

const explicitTag = forwardedArgs.some((arg) => arg === "--tag" || arg.startsWith("--tag="));
const prereleaseTag = getPrereleaseTag(packageJson.version);

if (prereleaseTag && !explicitTag) {
  npmArgs.push("--tag", prereleaseTag);
  console.log(`Publishing prerelease ${packageJson.version} with npm dist-tag "${prereleaseTag}".`);
} else {
  console.log(
