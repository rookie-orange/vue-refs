...
export const unplugin = createUnplugin<ForwardRefOptions | undefined>(
  (options = {}) => {
    ...
    function rememberImporter(child: string, importer: string): void {
      ...
      const importers =
        importerByChild.get(normalizedChild) ?? new Set<string>()
    ...
    return {
      ...
      vite: {
        configResolved(config) {
          ...
          if (!hasVuePlugin) {
            config.logger.warn(
              'vue-refx should be used with @vitejs/plugin-vue.',
            )
          }
        },
        ...
        async handleHotUpdate(ctx) {
          ...
          return importerModules.length > 0
            ? [...ctx.modules, ...importerModules]
            : undefined
        },
      },
      async transform(code, id) {
        ...
        const forwardedRefComponents = await resolveForwardedRefComponents(
          code,
          cleanId,
          this,
        )
...
async function resolveVueImport(
  ...
  const resolved = await context.resolve?.(componentImport.source, importer, {
    skipSelf: true,
  })
...
async function tryVueFile(filename: string): Promise<string | null> {
  const candidates = VUE_RE.test(filename)
    ? [filename]
    : [`${filename}.vue`, path.join(filename, 'index.vue')]
