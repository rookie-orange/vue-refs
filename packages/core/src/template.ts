...
export function transformTemplate(
  ...
  const localForwardedRefByName = new Map(
    localForwardedRefs.map((ref) => [ref.name, ref]),
  )
  ...
  walk(ast.children, (node) => {
    ...
    if (
      !shouldInjectForwardedRef(
        node.tag,
        node.tagType,
        options.forwardedRefComponents,
      )
    ) {
      return
    }
...
function shouldInjectForwardedRef(
  ...
  return getComponentAliases(tag).some((alias) =>
    forwardedRefComponents.has(alias),
  )
}
...
function hasForwardedRefBinding(
  props: Array<AttributeNode | DirectiveNode>,
): boolean {
  for (const prop of props) {
    if (
      prop.type === NodeTypes.ATTRIBUTE &&
      prop.name === FORWARDED_REF_PROP_NAME
    ) {
      return true
    }
...
function buildForwardedRefExpression(
  expression: string,
  _kind: RefBinding['kind'],
): string {
  if (
    isFunctionRefExpression(expression) ||
...
function isAssignableExpression(expression: string): boolean {
  return /^[A-Za-z_$][\w$]*(?:\s*(?:\.[A-Za-z_$][\w$]*|\[[^\]]+\]))*$/.test(
    expression.trim(),
  )
}
