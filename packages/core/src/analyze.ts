...
export function descriptorUsesDefineForwardRef(
...
export function scriptUsesDefineForwardRef(code: string): boolean {
  ...
  traverse(ast, {
    ImportDeclaration(path) {
      ...
      if (
        !FORWARDED_REF_IMPORT_SOURCES.has(node.source.value) ||
    ...
    CallExpression(path) {
      if (
        isDefineForwardRefCall(path, defineForwardRefLocals) &&
        isForwardingOverload(path.node)
      ) {
        found = true
        path.stop()
      }
