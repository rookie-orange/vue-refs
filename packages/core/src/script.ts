...
export function transformScriptSetup(
  ...
  const localForwardingBindings = forwardingBindings.filter(
  ...
  const propsIdentifier =
    defineProps?.propsIdentifier ?? choosePropsIdentifier(scope.bindings)
  const customRefIdentifier =
    scope.customRefIdentifier ?? chooseCustomRefIdentifier(scope.bindings)
  const refTypeIdentifier =
    scope.refTypeIdentifier ?? chooseRefTypeIdentifier(scope.bindings)
  const insertionPoint = Math.min(
    ...bindings.map((binding) => binding.statementStart),
  )
  ...
  const exposeReplacementBinding =
    exposedProperties.length > 0 && !defineExpose
      ? (bindings.find(
          (binding) => !binding.templateRef && binding.isExpressionStatement,
        ) ?? null)
      : null
  ...
  traverse(ast, {
    CallExpression(path) {
      ...
      const declarator =
        ...
        path.parentPath.node.init === node
      ...
      const statement =
        declarator?.parentPath?.node ?? expressionStatement?.node ?? node
      let bindingIdentifier = overload.templateRef
        ? getForwardedRefBindingIdentifier(
            declarator,
      ...
      if (
        overload.templateRef &&
        !bindingIdentifier &&
        overload.factorySource
      ) {
...
function parseDefineForwardRefOverload(
  ...
  if (!first) {
    throw new Error(
      'defineForwardRef() requires a template ref name or expose factory.',
    )
  }
  ...
  throw new Error(
    'defineForwardRef() expects a string literal or expose factory.',
  )
}
...
function getForwardedRefBindingIdentifier(
  ...
  throw new Error(
    'defineForwardRef() must be assigned to an identifier or used as a statement.',
  )
}
  ...
  traverse(ast, {
    CallExpression(path) {
      ...
      calls.push({
        ...
        declarationStart:
          declarator?.parentPath?.node.start ?? declarator?.node.start ?? null,
      })
    },
  })
...
function collectScriptScope(ast: File): ScriptScope {
  ...
  traverse(ast, {
    ...
    ImportDeclaration(path) {
      ...
      if (
        !isForwardedRefImportSource(node.source.value) ||
        node.importKind === 'type'
      ) {
        return
      }
...
function mergeDefinePropsType(
  ...
  const typeParameters = call.typeParameters as
    | TSTypeParameterInstantiation
    | undefined
...
function mergeDefineExpose(
  ...
  if (!argument) {
    s.appendLeft(
      offset + assertPosition(call.end) - 1,
      buildExposeObjectSource(properties),
    )
    return
  }
...
function appendObjectProperties(
  ...
  const insertionPoint =
    commaIndex >= 0 ? lastPropertyEnd + commaIndex + 1 : lastPropertyEnd
...
function removeDefineForwardRefImports(
  s: MagicString,
  offset: number,
  scope: ScriptScope,
): void {
  for (const declaration of scope.removableDefineForwardRefImports) {
    s.remove(
      offset + assertPosition(declaration.start),
      offset + assertPosition(declaration.end),
    )
  }
  ...
  for (const specifier of scope.removableDefineForwardRefSpecifiers) {
    ...
    if (nextSpecifier) {
      s.remove(
        offset + specifierStart,
        offset + assertPosition(nextSpecifier.start),
      )
    } else if (previousSpecifier) {
      s.remove(
        offset + assertPosition(previousSpecifier.end),
        offset + specifierEnd,
      )
    } else {
...
function getTypeParameterSource(
  ...
  return code
    .slice(assertPosition(parameter.start), assertPosition(parameter.end))
    .trim()
}
...
function buildForwardedRefPropType(
  ...
  const typeSources = [
    ...new Set(
      bindings
        .map((binding) => getForwardedRefValueType(binding))
        .filter(Boolean),
    ),
  ]
  ...
  return typeSources
    .map((type) => `${refTypeIdentifier}<${type} | null>`)
    .join(' | ')
}
...
function buildNextForwardedRefValueSource(
  binding: DefineForwardRefBinding,
): string {
  if (!binding.factorySource) {
    return 'nextValue'
  ...
  if (!binding.bindingIdentifier) {
    throw new Error(
      'defineForwardRef() with a factory requires a local ref binding.',
    )
  }
...
function isDefineForwardRefCall(
  ...
  return (
    callee.name === DEFINE_FORWARD_REF &&
    !path.scope.hasBinding(DEFINE_FORWARD_REF)
  )
}
